...
SELECT
  TRIM(P.APELLIDO) || ' ' || TRIM(P.NOMBRE) NOMBRE_COMPLETO_PERSONA, 
...
INNER JOIN (SELECT A.OBJECTID AGID, P.NRO_DOCUMENTO, TRIM(P.APELLIDO) || ' ' || TRIM(P.NOMBRE) NOMBRE_COMPLETO
            FROM ADELIA.PERSONA P
            INNER JOIN ADELIA.AGENTE A ON A.PERSONA = P.OBJECTID) AGENTE ON AGENTE.AGID = RAP.AGENTE
...
SELECT TRIM(P.APELLIDO) || ' ' || TRIM(P.NOMBRE) NOMBRE_COMPLETO_PERSONA, 
       ...
       AGENTE.NOMBRE_COMPLETO NOMBRE_COMPLETO_AGENTE,
       CASE 
            WHEN substr(RAP.OBJECTID,1,4) = '1063' THEN 'ATENCION FAMILIAR'
            WHEN substr(RAP.OBJECTID,1,4) = '0150' THEN 'HIJO/A'
            WHEN substr(RAP.OBJECTID,1,4) = '015A' THEN 'OTRO'
            WHEN substr(RAP.OBJECTID,1,4) = '0151' THEN 'PADRE/MADRE'
            WHEN substr(RAP.OBJECTID,1,4) = '014F' THEN 'CÓNYUGUE'
            WHEN substr(RAP.OBJECTID,1,4) = '4F91' THEN 'REPRESENTANTE'
        END AS VINCULO
FROM ADELIA.PERSONA P
INNER JOIN ADELIA.TIPO_DOCUMENTO TD ON TD.OBJECTID = P.TIPO_DOCUMENTO
INNER JOIN ADELIA.RELACION_AGENTE_PERSONA RAP ON RAP.PERSONA = P.OBJECTID
...
SELECT DISTINCT(TRIM(P.NRO_DOCUMENTO)), P.FECHA_NACIMIENTO
FROM ADELIA.PERSONA P
--INNER JOIN () FECHA ON FECHA.NRO_DOCUMENTO = TRIM(P.NRO_DOCUMENTO)
WHERE TRIM(P.NRO_DOCUMENTO) IN (SELECT TRIM(P.NRO_DOCUMENTO) NRO_DOCUMENTO
                                FROM ADELIA.PERSONA P
                                HAVING COUNT(TRIM(P.NRO_DOCUMENTO)) > 1
                                GROUP BY TRIM(P.NRO_DOCUMENTO))
      AND (TRIM(P.NRO_DOCUMENTO), P.FECHA_NACIMIENTO) NOT IN (
        SELECT TRIM(P.NRO_DOCUMENTO) NRO_DOCUMENTO, P.FECHA_NACIMIENTO
        FROM ADELIA.PERSONA P
        HAVING COUNT(TRIM(P.NRO_DOCUMENTO)) > 1
        GROUP BY TRIM(P.NRO_DOCUMENTO), P.FECHA_NACIMIENTO
      ) 
;
